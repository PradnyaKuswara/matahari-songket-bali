...
@section('content')
    <x-dashboard.page-title header="Management Customer" subtitle="Customer" :linkSubTitle="route('admin.dashboard.customers.index')" title="Index"
        :linkTitle="route('admin.dashboard.customers.index')"></x-dashboard.page-title>
            ...
            <div class="card bg-white shadow-lg rounded-lg text-center">
                <div class="card-body">
                    ...
                    <div id="table" class="overflow-x-scroll max-h-[28rem] 2xl:max-h-screen">
                        @include('pages.admin.customers.table')
                    </div>
                </div>
            </div>
        </div>
...
@push('scripts')
    ...
    <script>
        search("search", "table", "{{ url('admin/dashboard/customers/search?') }}")
    </script>
@endpush
