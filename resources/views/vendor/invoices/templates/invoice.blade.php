...
<body>
            ...
            <tr>
                <td class="border-0 pl-0" width="70%">
                    <h4 class="text-uppercase">
                        <strong>{{ $invoice->name }} <span style="color: #3C00DE;">{{ $invoice->getSerialNumber() }}</span>  </strong>
                    </h4>
                </td>
                <td class="border-0 pl-0">
                    @if ($invoice->status)
                        <div class="text-uppercase {{ $invoice->status == 'unpaid' ? 'red' : 'green' }}" style="font-size: 1.5rem; color:white; text-align: center;">
                            <strong>{{ $invoice->status }}</strong>
                        </div>
                    @endif
                    <p style="margin-top: 20px">{{ __('invoices::invoice.serial') }} <strong>{{ $invoice->getSerialNumber() }}</strong></p>
                    <p>{{ __('invoices::invoice.date') }}: <strong>{{ $invoice->getDate() }}</strong></p>
                    <p>
                        {{ __('invoices::invoice.pay_until') }}: <strong> {{ $invoice->getPayUntilDate() }}</strong>
                    </p>
                </td>
            </tr>
        </tbody>
    ...
    <table class="table table-items">
        ...
        <tbody>
            ...
            @endif
            <tr style="background-color: #3C00DE; color:white">
                <td colspan="{{ $invoice->table_columns - 2 }}" class="border-0"></td>
    ...
    <p>If you have any questions concerning this invoice, use the following contact
        information</p>
    <p>© {{ now()->format('Y') }} Matahari Songket Bali.</p>
